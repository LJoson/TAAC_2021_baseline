...
########## seg_extract - extracted features for scene segmentation
  ...
  if [ ! -d "${OUTPUT_SCENE_SEG_TRAIN_5K_DIR}" ]; then
    ...
    time python ShotDetect/shotdetect.py --video_dir ${DATASET_ROOT}/videos/train_5k_A/ \
  ...
  if [ ! -d ${OUTPUT_SCENE_SEG_TRAIN_AUDIO_FEAT_DIR} ]; then
    ...
    time python audio/extract_feat.py --data_root "${OUTPUT_SCENE_SEG_TRAIN_5K_DIR}"
  else
  ...
  if [ ! -d ${OUTPUT_SCENE_SEG_TRAIN_PLACE_FEAT_DIR} ]; then
    ...
    time python place/extract_feat.py --data_root "${OUTPUT_SCENE_SEG_TRAIN_5K_DIR}"
  else
...
########## seg_gt - generate gt files for scene segmentation training
  ...
  cd ${CODE_ROOT}/SceneSeg/pre || exit 1
  pwd
  time python gt_generator.py --video_dir ${DATASET_ROOT}/videos/train_5k/ \
...
########## seg_train
  ...
  time python run.py config/train5k.py
...
########## tag_gt - generate gt files for tagging training
  ...
  if [[ ! -f ${OUTPUT_TRAIN_FILE} ]] || [[ ! -f ${OUTPUT_VAL_FILE} ]]; then
    ...
    time python scripts/preprocess/generate_datafile.py --info_file ${DATASET_ROOT}/structuring/GroundTruth/structuring_tagging_info.txt \
...
########## tag_train
  ...
  python scripts/train_tagging.py --config "${CONFIG_FILE}"
...
########## test
  ...
  # OUTPUT_FILE_PATH as $2: optional, output file path
  OUTPUT_FILE_PATH="./results/structuring_tagging_5k.json"
  if [ -z "$2" ] ;then
      echo "[Warning] OUTPUT_FILE_PATH is not set, use default ${OUTPUT_FILE_PATH}"
  else
    OUTPUT_FILE_PATH="$2"
  fi
  # TEST_SCENE_VIDEOS_DIR as $3: optional, scene videos directory for test
  TEST_SCENE_VIDEOS_DIR=${DATASET_ROOT}/structuring/structuring_dataset_test_5k/scene_video
  if [ -z "$3" ] ;then
  ...
  if [ ! -d ${OUTPUT_SCENE_SEG_TEST_AUDIO_FEAT_DIR} ]; then
    ...
    time python audio/extract_feat.py --data_root "${OUTPUT_SCENE_SEG_TEST_5K_DIR}"
  else
  ...
  cd ${CODE_ROOT}/SceneSeg/lgss || exit 1
  pwd
  echo "[Info] begin to segment scene"
  time python run_inference.py --config config/common_test.py \
  ...
  python scripts/inference_for_structuring.py --model_pb "${CHECKPOINT_DIR}" \
...
########## evaluate
  ...
  time python scripts/structuring_evaluation.py --pred "${RESULT_FILE_PATH}" \
                                                --gt "${GT_FILE_PATH}"
  python scripts/structuring_evaluation.py --gt ../dataset/gt_json/test100.json --pred results/structuring_test5k_pred.json
