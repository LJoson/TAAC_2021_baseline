    ...
    def _get_single_item(self, ID):
        ...
        if 'place' in self.mode:
            for ind in self.shot_boundary_range:
                # print("!"*20+" shotid: {}".format(shotid))
                if int(shotid)+ind < 0:
                    name = 'shot_0000.npy'
                elif int(shotid)+ind > int(end_shot):
                    name = 'shot_{}.npy'.format(str(end_shot).zfill(4))
                else: 
                    name = 'shot_{}.npy'.format(strcal(shotid, ind))
                # print("!"*20+" npy : {}".format(name))
        ...
        if len(aud_feats) > 0:
            aud_feats = torch.stack(aud_feats)
        # print(type(place_feats), type(cast_feats), type(act_feats), type(aud_feats), type(label))
...
def main():
    ...
    cfg = Config.fromfile("./config/all.py")
