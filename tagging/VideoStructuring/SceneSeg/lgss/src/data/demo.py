...
def data_partition(cfg, valid_shotids):
    ...
    if len(valid_shotids) <= cfg.seq_len-1:
        ...
        for _ in range((cfg.seq_len-len(valid_shotids))//2):
            one_idxs.append({'imdbid': cfg.video_name, 'shotid': strcal(0, 0)})
        for i in range(len(valid_shotids)):
            one_idxs.append({'imdbid': cfg.video_name, 'shotid': strcal(i, 0)})
        remain = cfg.seq_len-len(one_idxs)
        for _ in range(remain):
            one_idxs.append({'imdbid': cfg.video_name, 'shotid': strcal(len(valid_shotids)-1, 0)})
        ...
        for i in range(seq_len_half-1, len(valid_shotids)-seq_len_half):
            ...
            for idx in range(-seq_len_half+1, seq_len_half+1):
                one_idxs.append({'imdbid': cfg.video_name, 'shotid': strcal(i, idx)})
            one_mode_idxs.append(one_idxs)
    '''    
    shotid_tmp = 0
