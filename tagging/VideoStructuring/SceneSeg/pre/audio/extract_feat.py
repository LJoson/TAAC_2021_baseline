...
if __name__ == '__main__':
    ...
    parser.add_argument('-nw','--num_workers', type=int,default=4,help='number of processors.')
    parser.add_argument('--duration_time',type=float,default=0.2)
    args = parser.parse_args()
    #args.list_file = osp.join(args.data_root,'meta/list_test.txt')
    args.list_file = None
    args.source_video_path = osp.join(args.data_root,"shot_split_video")
    args.save_wav_path = osp.join(args.data_root,"aud_wav")
    args.save_stft_path = osp.join(args.data_root,"aud_feat")
